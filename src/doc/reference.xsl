  ...
  <xsl:for-each select="
    ...
    <xsl:choose>
      <xsl:when test="@kind='class' or @kind='struct'">
        <xsl:if test="
            contains(compoundname, 'std::experimental::net::v1::') and
...
<xsl:template name="strip-net-ns">
  <xsl:param name="name"/>
  <xsl:choose>
    <xsl:when test="contains($name, 'std::experimental::net::v1::')">
      <xsl:value-of select="substring-after($name, 'std::experimental::net::v1::')"/>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="cleanup-type">
  <xsl:param name="name"/>
  <xsl:choose>
    <xsl:when test="contains($name, 'NET_TS_DECL ')">
      <xsl:value-of select="substring-after($name, 'NET_TS_DECL ')"/>
    </xsl:when>
    <xsl:when test="contains($name, 'NET_TS_DECL')">
      <xsl:value-of select="substring-after($name, 'NET_TS_DECL')"/>
    </xsl:when>
...
<xsl:template match="ref[@kindref='compound']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'experimental') or contains($name, 'std::experimental::net::v1::')">
      <xsl:variable name="dox-ref-id" select="@refid"/>
      <xsl:variable name="ref-name">
        <xsl:call-template name="strip-net-ns">
          <xsl:with-param name="name"
      ...
      <xsl:text>[link networking_ts.reference.</xsl:text>
...
<xsl:template match="ref[@kindref='member']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'namespacestd') and count($memberdefs) &gt; 0">
...
<xsl:text>    </xsl:text><xsl:if test="@prot='public'">public </xsl:if><xsl:call-template
 name="strip-net-ns"><xsl:with-param name="name" select="."/></xsl:call-template><xsl:if
...
<xsl:choose>
  <xsl:when test="count(name) &gt; 0">
    [[link networking_ts.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of select="name"/>
  ...
  <xsl:otherwise>
    ...
    [[link networking_ts.reference.<xsl:value-of select="$type-id"/>
...
<xsl:for-each select="sectiondef[@kind='public-func' or @kind='public-static-func']/memberdef" mode="class-table">
  ...
  [
    [[link networking_ts.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of select="$id"/>
...
</xsl:if>``[link networking_ts.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of
...
['Inherited from <xsl:call-template name="strip-net-ns">
...
<xsl:text>  </xsl:text><xsl:if test="string-length($stripped-type) &gt; 0"><xsl:value-of
 select="$stripped-type"/><xsl:text> </xsl:text></xsl:if>``[link networking_ts.reference.<xsl:value-of
